...
def page(**changes):
    values = {
        ...
        "visible_texts": ("나랑 라이더를 시작합니다.",),
        "navigation_targets": ("/start",),
...
def manifest(**changes):
    values = {
        "frontend_routes": frozenset({"/", "/start"}),
...
def audit(pages=None, value=None, benchmarks=()):
    return ArkaonExperienceOperationsAuditor().audit(
        report_id="r1",
...
def test_readability_content_heading_and_action_problems_are_proposed_immediately():
    bad = page(
        ...
        heading_levels=(2,),
        primary_actions=("a", "b"),
        visible_texts=("긴문장" * 40,),
        average_contrast_ratio=3.0,
        minimum_touch_target_px=32,
...
def test_broken_navigation_and_route_contract_mismatch_are_blockers():
    report = audit(
        pages=(page(navigation_targets=("/missing",)),),
    ...
    assert blockers
    assert {item.category for item in blockers} >= {"BROKEN_PATH", "FRONTEND_API"}
...
def test_unofficial_stale_or_copyable_benchmark_is_rejected():
    for bad in (
        replace(benchmark(), official=False),
        replace(benchmark(), expires_at=NOW),
