...
from apf.map_integration import (
...
def service(
    record: ProviderKnowledge | None = None,
) -> tuple[RiderNavigationHandoffService, Resolver]:
    resolver = Resolver()
...
def test_arkaon_search_compare_and_proposal_are_curated_and_non_activating() -> None:
    ...
    proposal = assistant.propose("certified-map", MapCapability.NAVIGATION_HANDOFF, NOW)
...
def test_stale_or_provisional_knowledge_fails_closed() -> None:
    for record in (
        ...
        with pytest.raises(KnowledgeRejected):
            registry(record).require("certified-map", MapCapability.NAVIGATION_HANDOFF, NOW)
