...
from apf.accuracy import AccuracyReport, AccuracyTask, NationalAccuracyVerdict
from apf.arkaon import ARKAON_DOMAIN, ArkaonCapability, ArkaonProfile, ForbiddenAuthority
from apf.branch_registry import Branch, BranchRegistry, BranchStatus, BranchType, ServiceArea
from apf.rollout import (
    ArkaonRelease,
...
def profile():
    return ArkaonProfile(
        profile_id="apf-arkaon-v1",
        domain=ARKAON_DOMAIN,
        version=1,
...
def test_kill_switch_pauses_release_and_every_branch() -> None:
    ...
    assert value.get_deployment("eta-release-1", "sejong-hub").stage is RolloutStage.PAUSED
    assert (
        value.get_deployment("eta-release-1", "seoul-hub").fallback_policy_id == "public-eta-v1"
    )
...
def test_one_branch_can_rollback_without_disabling_other_branch() -> None:
    ...
    assert value.get_deployment("eta-release-1", "seoul-hub").stage is RolloutStage.PILOT
