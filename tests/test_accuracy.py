...
def test_sparse_or_unbalanced_branch_data_cannot_pass() -> None:
    evaluator = ArkaonAccuracyEvaluator()
    sparse = tuple(replace(item, segment="branch-sejong") for item in passing_observations())
    report = evaluator.evaluate(
        policy=policy(),
        model_version="eta-v1",
        observations=sparse,
...
def test_financial_tasks_require_exact_integer_results() -> None:
    ...
    inaccurate = tuple(
        replace(item, predicted=item.actual + 1) for item in passing_observations()
    )
