        ...
        capabilities=frozenset(
            {
                ...
                ArkaonCapability.PATTERN_CANDIDATE_ANALYSIS,
            }
        ),
...
def directive(*, capability=ArkaonCapability.CODE_CHANGE_PROPOSAL):
    return authority().issue(
        ...
        allowed_paths=(
            ...
            "docs/33-arkaon-accuracy-assurance.md",
            "knowledge/readiness/v0.1-readiness.json",
        ),
        acceptance_criteria=(
...
def test_profile_rejects_foreign_domain() -> None:
    with pytest.raises(ValueError, match="ARKAON_DOMAIN_MISMATCH"):
        ArkaonProfile(
            profile_id="unsafe",
            domain="NARANG_RIDER",
            version=1,
...
def test_tampered_or_expired_directive_cannot_authorize_work() -> None:
    ...
    with pytest.raises(ValueError, match="INVALID_DIRECTIVE_SIGNATURE"):
        coordinator.propose_change(
            ...
            touched_paths=("src/apf/accuracy.py",),
            change_digest="a" * 64,
            summary="tampered",
...
def test_arkaon_can_propose_but_cannot_merge_or_deploy() -> None:
    ...
    replay = coordinator.propose_change(
        proposal_id="proposal-1",
        directive=directive(),
        touched_paths=("docs/33-arkaon-accuracy-assurance.md",),
        change_digest="a" * 64,
    ...
    with pytest.raises(
        ValueError, match="ARKAON_FORBIDDEN_AUTHORITY:AUTOMATIC_OWNED_ASSET_PROMOTION"
    ):
        coordinator.assert_authority_allowed("AUTOMATIC_OWNED_ASSET_PROMOTION")
...
def test_code_proposal_cannot_exceed_directive_or_run_arbitrary_commands() -> None:
    ...
    with pytest.raises(ValueError, match="PROPOSAL_OUTSIDE_DIRECTIVE_SCOPE"):
        coordinator.propose_change(
            ...
            directive=directive(),
            touched_paths=("src/apf/payments.py",),
            change_digest="b" * 64,
