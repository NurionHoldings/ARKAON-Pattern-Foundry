"""ARKAON accuracy, fallback, national gates, and drift detection.

Integer metrics only. Financial tasks require exactness. A failed gate never
activates a model; public-rule fallback or human review is used instead.
"""
...
class AccuracyTask(StrEnum):
    ...
    PATTERN_REUSE_QUALITY = "PATTERN_REUSE_QUALITY"
...
class ArkaonAccuracyEvaluator:
    def evaluate(
        ...
        *,
        ...
        if any(
            len(errors) < policy.min_samples_per_segment for errors in segment_errors.values()
        ):
            failures.append("INSUFFICIENT_SEGMENT_SAMPLES")
        segment_maes = [
            sum(errors) // len(errors) for errors in segment_errors.values() if errors
        ]
        best_segment = min(segment_maes)
        worst_segment = max(segment_maes)
        segment_ratio_bps = (
            10_000 if worst_segment == 0 else worst_segment * 10_000 // max(1, best_segment)
        )
...
def evaluate_national_accuracy(
    *,
    ...
    passed = not missing and not failed and ratio <= max_cross_branch_mae_ratio_bps
