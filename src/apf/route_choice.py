"""Provider-neutral route choice with rider authority, safety, and pay protection.

Shortest distance is never forced. Predicted-route deviation is never a penalty.
Transplanted analog of NARANG RIDER feat/068 for Pattern Foundry.
"""
...
@dataclass(frozen=True)
class MultiRouteComparison:
    explanations: tuple[RouteExplanation, ...]
    recommended_route_id: str | None
    shortest_route_id: str
    shortest_forced: bool = False
    auto_selected: bool = False
    deviation_penalty_allowed: bool = False
    rider_must_choose: bool = True
...
class DeviationAssessment:
    review_hint: bool
    misconduct_evidence: bool
    automatic_penalty_allowed: bool
    pay_change_allowed: bool
    explanation: str
...
class RouteChoiceService:
    ...
    def compare(
        ...
        explanations = self.evaluate(alternatives, objective, costs, accepted, now)
        shortest = min(alternatives, key=lambda route: (route.distance_m, route.duration_s))
        recommended = explanations[0].route_id if explanations else None
        return MultiRouteComparison(
            explanations=explanations,
            recommended_route_id=recommended,
            shortest_route_id=shortest.route_id,
            shortest_forced=False,
            auto_selected=False,
            deviation_penalty_allowed=False,
            rider_must_choose=True,
        )
    ...
    def _explain(
        ...
        reasons = (
            ...
            "추천일 뿐이며 라이더가 최종 선택합니다",
            "예상경로 이탈은 제재 신호가 아닙니다",
        )
    ...
    @staticmethod
    def assess_deviation(*, planned_distance_m: int, observed_distance_m: int) -> DeviationAssessment:
        if min(planned_distance_m, observed_distance_m) < 0:
            raise RouteRejected("invalid distance")
