...
def public_profile() -> HTMLResponse:
    ...
    name = escape(os.getenv("MACOL_PROFILE_NAME", "__MACOL_PROFILE_NAME__")[:80])
    intro = escape(os.getenv("MACOL_PROFILE_INTRO", "__MACOL_PROFILE_INTRO__")[:500])
    menus = [escape(s.strip()[:80]) for s in os.getenv(
        "MACOL_PROFILE_MENUS", "__MACOL_PROFILE_MENUS__"
    ).split(",") if s.strip()][:8]
