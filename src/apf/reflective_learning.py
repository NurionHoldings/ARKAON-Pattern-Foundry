...
class ReflectiveLesson:
    ...
    def to_document(self) -> dict[str, object]:
        return {
            ...
            "failure_or_rejection_reason": self.failure_or_rejection_reason,
            "evidence_digest": self.evidence_digest,
            "recorded_at": self.recorded_at.isoformat(),
            "contains_operational_data": self.contains_operational_data,
            "immutable": True,
        }
...
class ArkaonReflectiveLearning:
    ...
    _NEGATIVE_OUTCOMES = frozenset({Decision.REJECT, Decision.DEFER, Decision.REGRESSION})
    ...
    def assess_self(self, case_id: str, assessment: SelfAssessment) -> ReflectionCase:
        ...
        if not all(
            (
                assessment.platform_id,
                assessment.current_capability,
                assessment.strength,
                assessment.gap,
            ...
            raise ReflectionRejected("complete self-assessment required")
    ...
    def eternian_review(self, case_id: str, *, review_digest: str) -> ReflectionCase:
        ...
        updated = replace(
            ...
            stage=ReflectionStage.ETERNIAN_REVIEWED,
        )
    ...
    def record_lesson(
        ...
        *,
        ...
        if (
            lesson_id in self._lessons
            or not lesson_id
            or not principle.strip()
            or not reusable_pattern.strip()
            or now.tzinfo is None
            or (
                value.operator_decision in self._NEGATIVE_OUTCOMES
        ...
        evidence = self._digest(
            (
                case_id,
                value.operator_decision.value if value.operator_decision else "",
                principle,
                reusable_pattern,
        ...
        platform_scope = value.assessment.platform_id if value.assessment else "UNKNOWN"
        combined = f"{principle} {reusable_pattern} {failure_or_rejection_reason}".casefold()
        forbidden = ("member", "order", "location", "payment", "settlement", "identity", "pii")
        if any(token in combined for token in forbidden):
            raise ReflectionRejected("operational or identity data cannot enter reflective memory")
        lesson = ReflectiveLesson(
            lesson_id,
            case_id,
            platform_scope,
            value.operator_decision or Decision.DEFER,
            principle,
            reusable_pattern,
            failure_or_rejection_reason,
            evidence,
    ...
    def modify_model_weights(self) -> None:
        raise ReflectionRejected("model weight self-change is forbidden")

    def relax_operational_standards(self) -> None:
        raise ReflectionRejected("operational standard self-relaxation is forbidden")

    def modify_operational_prompt(self) -> None:
        raise ReflectionRejected("operational prompt self-change is forbidden")

    def self_approve_proposal(self) -> None:
        raise ReflectionRejected("self-approval is forbidden")

    def apply_lesson_to_operational_code(self) -> None:
        raise ReflectionRejected("automatic operational application is forbidden")

    def auto_merge_or_deploy(self) -> None:
        raise ReflectionRejected("automatic merge or deploy is forbidden")

    def _require(self, case_id: str, stage: ReflectionStage) -> ReflectionCase:
        if case_id not in self._cases:
            raise ReflectionRejected("reflection case not found")
    ...
    def _save(self, value: ReflectionCase, action: str, evidence: str) -> ReflectionCase:
        ...
        self.events.append(
            ReflectionEvent(sequence, value.case_id, action, evidence, previous, event_digest)
        )
        return value
