"""ARKAON evolution boundary: propose-only development and gated promotion.

Transplanted analog of NARANG RIDER `arkaon.py` for Pattern Foundry. ARKAON may
propose changes and candidates; it cannot merge, deploy, or exercise forbidden
authorities.
"""
...
ARKAON_DOMAIN = "ARKAON_PATTERN_FOUNDRY"
...
class ArkaonCapability(StrEnum):
    ...
    PATTERN_CANDIDATE_ANALYSIS = "PATTERN_CANDIDATE_ANALYSIS"
...
class ForbiddenAuthority(StrEnum):
    ...
    AUTOMATIC_OWNED_ASSET_PROMOTION = "AUTOMATIC_OWNED_ASSET_PROMOTION"
...
class EthernianDirectiveAuthority:
    ...
    def _validate_paths(paths: tuple[str, ...]) -> tuple[str, ...]:
        ...
        allowed_roots = ("src/", "tests/", "docs/", "knowledge/", "config/", "api/", "schemas/")
...
class ArkaonDevelopmentCoordinator:
    ...
    def propose_change(
        ...
        *,
        ...
        if len(change_digest) != 64 or any(
            character not in "0123456789abcdef" for character in change_digest
        ):
