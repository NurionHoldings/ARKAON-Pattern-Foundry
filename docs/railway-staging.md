...
For the web service, set the Railway healthcheck path to `/ready` when this change is on the deployed commit. `/health` only checks that the process responds. `/ready` returns 503 until the owner OAuth configuration, `/data` mount and directories, PostgreSQL migration ledger, and configured tenant all exist. It reads those resources without migrating or provisioning anything, and reveals no secret or database error in the response. Passing `/ready` does not replace owner sign-in, non-owner denial, backup restoration, product readiness review, or an approved release SHA.
