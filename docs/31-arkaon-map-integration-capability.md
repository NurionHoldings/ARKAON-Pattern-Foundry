# ARKAON 지도 연동 역량 및 내비게이션 인계 (Pattern Foundry)

조회일: 2026-09-16
상태: NARANG RIDER `feat/065-arkaon-map-integration-capability`에서 이식 완료.
실제 공급자 인증·키·Sandbox·법률 검토는 **BLOCKED**. 공개 카탈로그 30건은 변경하지 않음.

## 이식 범위

| 원본 (NARANG RIDER) | Pattern Foundry |
|---|---|
| `src/narang_rider/map_integration.py` | `src/apf/map_integration.py` |
| `tests/test_map_integration.py` | `tests/test_map_integration.py` |
| `config/map-provider-knowledge.provisional.json` | `config/map-provider-knowledge.provisional.json` |
| `api/navigation-handoff.openapi.yaml` | `api/navigation-handoff.openapi.yaml` |

라이브 지도 API, 키, 좌표 저장, 공급자 URL scheme 하드코딩은 이식하지 않는다.
